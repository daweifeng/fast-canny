...
__m256d simd_approx_atan(__m256d x) {
    // Constants for approximation (coefficients would be based on a polynomial fit)
...
__m256d simd_atan2(__m256d y, __m256d x) {
    ...
    __m256d atan_result = simd_approx_atan(tangent); // Substitute with approximate atan
    ...
    __m256d angle_offset = _mm256_blendv_pd(pi, -pi, y_ge_zero); // +/- pi based on y sign
    __m256d adjusted_result = _mm256_add_pd(atan_result, _mm256_and_pd(x_lt_zero, angle_offset));
...
void Gradient(double *input, double *output, double *theta, int width, int height) {
    ...
    double *paddedInput = new double[(width + 2 * halfSize) * (height + 2 * halfSize)];
    ...
    __m256d sum1_x, sum2_x, sum3_x, sum4_x, sum5_x, sum6_x, sum7_x, sum8_x, sum9_x, sum10_x;
    __m256d sum1_y, sum2_y, sum3_y, sum4_y, sum5_y, sum6_y, sum7_y, sum8_y, sum9_y, sum10_y;
        ...
        pixels5 = _mm256_setzero_pd();
            ...
            for (int k = -padd; k <= padd; k++) {
                for (int l = -padd; l <= padd; l++) {
                    ...
                    pixels1 = _mm256_loadu_pd(&paddedInput[((idx) / width + halfSize + k) * paddedWidth +
                         ((idx) % width + halfSize + l)]);
                    pixels2 = _mm256_loadu_pd(&paddedInput[((idx+4) / width + halfSize + k) * paddedWidth +
                         ((idx+4) % width + halfSize + l)]);
                    pixels3 = _mm256_loadu_pd(&paddedInput[((idx+8) / width + halfSize + k) * paddedWidth +
                         ((idx+8) % width + halfSize + l)]);
                    pixels4 = _mm256_loadu_pd(&paddedInput[((idx+12) / width + halfSize + k) * paddedWidth +
                         ((idx+12) % width + halfSize + l)]);
                    pixels5 = _mm256_loadu_pd(&paddedInput[((idx+16) / width + halfSize + k) * paddedWidth +
                    ...
                    pixels1 = _mm256_loadu_pd(&paddedInput[((idx+20) / width + halfSize + k) * paddedWidth +
                         ((idx+20) % width + halfSize + l)]);
                    pixels2 = _mm256_loadu_pd(&paddedInput[((idx+24) / width + halfSize + k) * paddedWidth +
                         ((idx+24) % width + halfSize + l)]);
                    pixels3 = _mm256_loadu_pd(&paddedInput[((idx+28) / width + halfSize + k) * paddedWidth +
                         ((idx+28) % width + halfSize + l)]);
                    pixels4 = _mm256_loadu_pd(&paddedInput[((idx+32) / width + halfSize + k) * paddedWidth +
                         ((idx+32) % width + halfSize + l)]);
                    pixels5 = _mm256_loadu_pd(&paddedInput[((idx+36) / width + halfSize + k) * paddedWidth +
            ...
            grad1 = _mm256_add_pd(
                _mm256_mul_pd(sum1_x, _mm256_set1_pd(0.5)),
                _mm256_mul_pd(sum1_y, _mm256_set1_pd(0.5))
            );
            grad2 = _mm256_add_pd(
                _mm256_mul_pd(sum2_x, _mm256_set1_pd(0.5)),
                _mm256_mul_pd(sum2_y, _mm256_set1_pd(0.5))
            );
            grad3 = _mm256_add_pd(
                _mm256_mul_pd(sum3_x, _mm256_set1_pd(0.5)),
                _mm256_mul_pd(sum3_y, _mm256_set1_pd(0.5))
            );
            grad4 = _mm256_add_pd(
                _mm256_mul_pd(sum4_x, _mm256_set1_pd(0.5)),
                _mm256_mul_pd(sum4_y, _mm256_set1_pd(0.5))
            );
            grad5 = _mm256_add_pd(
                _mm256_mul_pd(sum5_x, _mm256_set1_pd(0.5)),
                _mm256_mul_pd(sum5_y, _mm256_set1_pd(0.5))
            );
            grad6 = _mm256_add_pd(
                _mm256_mul_pd(sum6_x, _mm256_set1_pd(0.5)),
                _mm256_mul_pd(sum6_y, _mm256_set1_pd(0.5))
            );
            grad7 = _mm256_add_pd(
                _mm256_mul_pd(sum7_x, _mm256_set1_pd(0.5)),
                _mm256_mul_pd(sum7_y, _mm256_set1_pd(0.5))
            );
            grad8 = _mm256_add_pd(
                _mm256_mul_pd(sum8_x, _mm256_set1_pd(0.5)),
                _mm256_mul_pd(sum8_y, _mm256_set1_pd(0.5))
            );
            grad9 = _mm256_add_pd(
                _mm256_mul_pd(sum9_x, _mm256_set1_pd(0.5)),
                _mm256_mul_pd(sum9_y, _mm256_set1_pd(0.5))
            );
            grad10 = _mm256_add_pd(
                _mm256_mul_pd(sum10_x, _mm256_set1_pd(0.5)),
            ...
            _mm256_storeu_pd(&output[((idx+4) / width) * width + ((idx+4) % width)], grad2);
            _mm256_storeu_pd(&output[((idx+8) / width) * width + ((idx+8) % width)], grad3);
            _mm256_storeu_pd(&output[((idx+12) / width) * width + ((idx+12) % width)], grad4);
            _mm256_storeu_pd(&output[((idx+16) / width) * width + ((idx+16) % width)], grad5);
            _mm256_storeu_pd(&output[((idx+20) / width) * width + ((idx+20) % width)], grad6);
            _mm256_storeu_pd(&output[((idx+24) / width) * width + ((idx+24) % width)], grad7);
            _mm256_storeu_pd(&output[((idx+28) / width) * width + ((idx+28) % width)], grad8);
            _mm256_storeu_pd(&output[((idx+32) / width) * width + ((idx+32) % width)], grad9);
            _mm256_storeu_pd(&output[((idx+36) / width) * width + ((idx+36) % width)], grad10);
            _mm256_storeu_pd(&theta[((idx) / width) * width + ((idx) % width)], dir1);
            _mm256_storeu_pd(&theta[((idx+4) / width) * width + ((idx+4) % width)], dir2);
            _mm256_storeu_pd(&theta[((idx+8) / width) * width + ((idx+8) % width)], dir3);
            _mm256_storeu_pd(&theta[((idx+12) / width) * width + ((idx+12) % width)], dir4);
            _mm256_storeu_pd(&theta[((idx+16) / width) * width + ((idx+16) % width)], dir5);
            _mm256_storeu_pd(&theta[((idx+20) / width) * width + ((idx+20) % width)], dir6);
            _mm256_storeu_pd(&theta[((idx+24) / width) * width + ((idx+24) % width)], dir7);
            _mm256_storeu_pd(&theta[((idx+28) / width) * width + ((idx+28) % width)], dir8);
            _mm256_storeu_pd(&theta[((idx+32) / width) * width + ((idx+32) % width)], dir9);
            _mm256_storeu_pd(&theta[((idx+36) / width) * width + ((idx+36) % width)], dir10);
...
void GradientSlow(double *input, double *output, double *theta, int width, int height) {
   int padd = 1;
    ...
    // Example: Modify the padding logic to include the boundary or use zero padding.
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            ...
            for (int k_i = -padd; k_i <= padd; k_i++) {
                for (int k_j = -padd; k_j <= padd; k_j++) {
                    ...
                    // Apply zero-padding if the index is out of bounds
                    if (ni >= 0 && ni < height && nj >= 0 && nj < width) {
                        ...
                        grad_x += 0 * sobel_x[k_i + padd][k_j + padd];
                        grad_y += 0 * sobel_y[k_i + padd][k_j + padd];
                    }
                }
            }

        int sobel_index = i * width + j; // Index in 1D output arrays
        output[sobel_index] = grad_x/2 + grad_y/2;
        theta[sobel_index] = atan2(grad_y, grad_x);
        }
    }
